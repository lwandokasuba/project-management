...
func (pq *ProjectQuery) QueryAssignments() *AssignmentQuery {
	query := (&AssignmentClient{config: pq.config}).Query()
...
func (pq *ProjectQuery) WithAssignments(opts ...func(*AssignmentQuery)) *ProjectQuery {
	query := (&AssignmentClient{config: pq.config}).Query()
...
func (pq *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := pq.withAssignments; query != nil {
		if err := pq.loadAssignments(ctx, query, nodes,
			func(n *Project) { n.Edges.Assignments = []*Assignment{} },
			func(n *Project, e *Assignment) { n.Edges.Assignments = append(n.Edges.Assignments, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProjectQuery) loadAssignments(ctx context.Context, query *AssignmentQuery, nodes []*Project, init func(*Project), assign func(*Project, *Assignment)) error {
	...
	query.Where(predicate.Assignment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.AssignmentsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.project_assignments
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_assignments" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_assignments" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
