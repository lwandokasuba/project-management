...
func (sq *StatementQuery) QueryAssignment() *AssignmentQuery {
	query := (&AssignmentClient{config: sq.config}).Query()
...
func (sq *StatementQuery) Clone() *StatementQuery {
	...
	return &StatementQuery{
		...
		withAssignment: sq.withAssignment.Clone(),
...
func (sq *StatementQuery) WithAssignment(opts ...func(*AssignmentQuery)) *StatementQuery {
	query := (&AssignmentClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withAssignment = query
	return sq
}
...
//		GroupBy(statement.FieldCreateTime).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Statement.Query().
//		Select(statement.FieldCreateTime).
//		Scan(ctx, &v)
...
func (sq *StatementQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Statement, error) {
	var (
		...
		_spec       = sq.querySpec()
		loadedTypes = [1]bool{
			sq.withAssignment != nil,
		}
	)
	if sq.withAssignment != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := sq.withAssignment; query != nil {
		if err := sq.loadAssignment(ctx, query, nodes, nil,
			func(n *Statement, e *Assignment) { n.Edges.Assignment = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *StatementQuery) loadAssignment(ctx context.Context, query *AssignmentQuery, nodes []*Statement, init func(*Statement), assign func(*Statement, *Assignment)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Statement)
	for i := range nodes {
		if nodes[i].assignment_statements == nil {
			continue
		}
		fk := *nodes[i].assignment_statements
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "assignment_statements" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
