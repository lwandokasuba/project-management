...
func (uq *UserQuery) QueryAssignments() *AssignmentQuery {
	query := (&AssignmentClient{config: uq.config}).Query()
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withAssignments: uq.withAssignments.Clone(),
...
func (uq *UserQuery) WithAssignments(opts ...func(*AssignmentQuery)) *UserQuery {
	query := (&AssignmentClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withAssignments = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withAssignments; query != nil {
		if err := uq.loadAssignments(ctx, query, nodes,
			func(n *User) { n.Edges.Assignments = []*Assignment{} },
			func(n *User, e *Assignment) { n.Edges.Assignments = append(n.Edges.Assignments, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadAssignments(ctx context.Context, query *AssignmentQuery, nodes []*User, init func(*User), assign func(*User, *Assignment)) error {
